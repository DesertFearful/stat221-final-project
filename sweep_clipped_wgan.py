...
def save_sweep_grid(real_samples, results, output_path):
    weight_clips = sorted({result["weight_clip"] for result in results})
    n_critics = sorted({result["n_critic"] for result in results})
    results_map = {(result["weight_clip"], result["n_critic"]): result for result in results}
    ...
    for row_index, weight_clip in enumerate(weight_clips):
        for col_index, n_critic in enumerate(n_critics):
            ...
            axis.set_title(f"clip={weight_clip:g}, n_critic={n_critic}")
            axis.text(
                0.03,
                0.97,
    ...
    figure.suptitle("Clipped WGAN Sweep: real samples in gray, generated samples in blue")
...
def save_summary_heatmaps(results, output_path):
    ...
    figure.suptitle("Clipped WGAN Sweep Summary")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Sweep weight clipping and n_critic for the 2D Gaussian WGAN experiment.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/clipped_wgan_sweep"))
    parser.add_argument("--weight-clips", type=float, nargs="+", default=[0.01, 0.05, 0.1])
...
def main():
    ...
    results = []
    run_index = 0
    total_runs = len(args.weight_clips) * len(args.n_critics)

    for weight_clip in args.weight_clips:
        for n_critic in args.n_critics:
            run_index += 1
            run_name = f"clip_{weight_clip:g}_ncritic_{n_critic}"
            ...
            metrics["weight_clip"] = weight_clip
